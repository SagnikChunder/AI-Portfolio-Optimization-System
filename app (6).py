...
# Large pool of liquid, well-known US stocks across sectors
TICKER_POOL = [
    # Technology
    "AAPL", "MSFT", "GOOGL", "AMZN", "META", "NVDA", "TSLA", "ADBE", "CRM", "INTC",
    "AMD", "QCOM", "TXN", "AVGO", "ORCL", "IBM", "CSCO", "AMAT", "MU", "KLAC",
    # Finance
    "JPM", "BAC", "WFC", "GS", "MS", "BLK", "AXP", "USB", "PNC", "SCHW",
    # Healthcare
    "JNJ", "UNH", "PFE", "ABBV", "MRK", "TMO", "ABT", "DHR", "BMY", "AMGN",
    # Consumer
    "WMT", "HD", "MCD", "NKE", "SBUX", "TGT", "COST", "LOW", "TJX", "YUM",
    # Energy
    "XOM", "CVX", "COP", "SLB", "EOG", "MPC", "PSX", "VLO", "OXY", "HAL",
    # Industrials
    "BA", "CAT", "GE", "HON", "MMM", "LMT", "RTX", "UPS", "FDX", "DE",
    # Communication
    "T", "VZ", "NFLX", "DIS", "CMCSA", "CHTR", "TMUS", "FOX",
    # Materials & Utilities
    "LIN", "APD", "ECL", "NEM", "FCX", "NEE", "DUK", "SO", "AEP", "EXC",
]

def get_random_tickers(n=5):
    """Pick n tickers randomly from the pool on each app load."""
    return random.sample(TICKER_POOL, n)

DEFAULT_TICKERS = get_random_tickers(5)
...
def create_allocation_table(optimal, tickers, investment_amount):
    ...
    for ticker in tickers:
        ...
        rows.append({
            ...
            f"Allocation (INR)": format_currency(allocation),
        })
    return rows
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        dcc.Input(id="ticker-input", type="text", value=", ".join(DEFAULT_TICKERS), style={"flex": "1", "padding": "10px"}),
                        html.Button("🎲 Randomise", id="randomise-button", n_clicks=0, style={"padding": "10px 16px", "backgroundColor": "#6366f1", "color": "white", "border": "none", "borderRadius": "8px", "fontWeight": "600", "cursor": "pointer", "whiteSpace": "nowrap"}),
                    ],
                ),
                html.P(f"Randomly selected from a pool of {len(TICKER_POOL)} stocks across 8 sectors. Edit freely or hit Randomise.", style={"fontSize": "12px", "color": "#6b7280", "marginTop": "-8px", "marginBottom": "12px"}),
                html.Div(
...
def randomise_tickers(_):
    return ", ".join(get_random_tickers(5))
            ...
            html.Div(
                ...
                children=[
                    html.Div([html.H3("Allocation Plan"), dash_table.DataTable(
                        columns=[{"name": col, "id": col} for col in ["Ticker", "Weight (%)", f"Allocation ({CURRENCY_SYMBOL})".replace(CURRENCY_SYMBOL, "INR")]],
