...
CURRENCY_SYMBOL = '₹'
CURRENCY_LOCALE = 'en_IN' # For Indian number formatting (e.g., crores, lakhs, but using standard commas here for simplicity)

# Set locale for rupee formatting
try:
    locale.setlocale(locale.LC_ALL, CURRENCY_LOCALE)
except locale.Error:
    # Fallback if the specific locale is not available
    print("Warning: Indian locale not available. Falling back to default formatting.")
    pass

def format_currency(amount):
    """Formats a number with the Rupee symbol and appropriate commas."""
    # Using a simple string format to ensure compatibility across environments
    # For robust Indian formatting (lakhs/crores), a custom function or proper locale is needed.
    return f"{CURRENCY_SYMBOL}{amount:,.0f}" if amount >= 1 else f"{CURRENCY_SYMBOL}{amount:,.2f}"
...
def create_efficient_frontier_plot(df, optimal_portfolio, tickers):
    ...
    # This function remains identical as the plots are based on unitless percentages/ratios.
...
def create_correlation_heatmap(returns, tickers):
    ...
    # This function remains identical as it uses unitless correlation coefficients.
...
def create_performance_plot(data, optimal_portfolio, tickers):
    ...
    # This function remains identical as it uses normalized values (base 100).
...
def generate_investment_report(optimal_portfolio, tickers, investment_amount):
    """Generate detailed investment report and summary markdown in Rupees"""
    ...
    for i, ticker in enumerate(tickers):
        if weights[i] > 0.001: # Only include significant weights
            ...
            report_data.append({
                ...
                'Allocation ({})'.format(CURRENCY_SYMBOL): format_currency(allocations[i]), # CHANGED
                'Current Price ({})'.format(CURRENCY_SYMBOL): format_currency(price), # CHANGED
...
app.layout = html.Div(style={'maxWidth': '1200px', 'margin': '0 auto', 'padding': '20px'}, children=[
    ...
    html.Div(className='config-panel', style={'padding': '20px', 'border': '1px solid #ddd', 'borderRadius': '5px', 'marginBottom': '20px'}, children=[
        ...
        html.Div([
            # CHANGED Label to reflect Rupee
            html.Label(f"Investment Amount ({CURRENCY_SYMBOL}):"), 
            # Changed default value to something more typical for Rupee/Indian context (e.g., 1,00,000)
            dcc.Input(id='investment-amount', type='number', value=100000, min=1, style={'width': '100%'}) 
        ], style={'marginBottom': '20px'}),
    ...
    dcc.Loading(id="loading-1", type="default", children=[
        ...
        dash_table.DataTable(
            ...
            columns=[
                ...
                # CHANGED Column names in DataTable
                {"name": f"Allocation ({CURRENCY_SYMBOL})", "id": f"Allocation ({CURRENCY_SYMBOL})"}, 
                {"name": f"Current Price ({CURRENCY_SYMBOL})", "id": f"Current Price ({CURRENCY_SYMBOL})"},
